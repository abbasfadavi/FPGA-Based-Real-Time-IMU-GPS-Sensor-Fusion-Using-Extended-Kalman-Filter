...
//					S[i][j]+=H_gps[i][k]*P[k][l]*H_gps[j][l];
//
//		fp R_gps[6] = {5.169,5.169,5.169,0.0051,0.0051,0.0051};
//
//		for(int i=0;i<6;i++)
//			for(int j=0;j<6;j++)
//				if(i == j)S[i][j] = S[i][j] + R_gps[i];
//
//	fp Y[6][STATE_SIZE]={0};
...
//	loop4 : for(int i=0;i<STATE_SIZE;i++)
//		for(int j=0;j<6;j++)
//			K[i][j]=Kt[j][i];
//
//	fp w3[22][22]={0};
...
//				w6[i]+=K[i][j]*z_gps[j];
//
//	for(int i=0;i<22;i++)state[i] = state[i] + w6[i];
//
//	fp H_mag[3][22];
...
//	for(int i=0;i<3;i++)
//		for(int j=0;j<3;j++)
//			if(i == j)SS[i][i] = w8[i][i] + Rmag;
//
//	fp XX[22][3]={0};
//	for(int i=0;i<22;i++)
//		for(int j=0;j<3;j++)
//			for(int k=0;k<22;k++)
//					XX[i][j]+=P[i][k]*H_mag[j][k];
//
//	fp YY[3][22]={0};
//	for(int i=0;i<3;i++)
//		for(int j=0;j<22;j++)
...
{
#pragma HLS ARRAY_PARTITION variable=in_mat complete
	int k = 0;
	for (int i = 0; i < 22; i++)
	{
		state[i] = in_mat[k][block];
		k++;
	}
	for (int i = 0; i < 22; i++)
		for (int j = 0; j < 22; j++)
		{
			p[i][j] = in_mat[k][block];
			k++;
		}
	for (int i = 0; i < 160; i++)
		for (int j = 0; j < 3; j++)
		{
			accel_block[i][j] = in_mat[k][block];
			k++;
		}
	for (int i = 0; i < 160; i++)
	{
	...
	for (int i = 0; i < 160; i++)
		for (int j = 0; j < 3; j++)
		{
			mag_block[i][j] = in_mat[k][block];
			k++;
		}
	for (int i = 0; i < 3; i++)
	{
		gps_pos[i] = in_mat[k][block];
		k++;
	}
	for (int i = 0; i < 3; i++)
	{
		gps_vel[i] = in_mat[k][block];
		k++;
	}
}
//
...
{
#pragma HLS ARRAY_PARTITION variable=out_mat complete
	int k = 0;
	for (int i = 0; i < 22; i++)
	{
		state_out[i] = out_mat[k][block];
		k++;
	}
	for (int i = 0; i < 22; i++)
		for (int j = 0; j < 22; j++)
		{
			p_out[i][j] = out_mat[k][block];
			k++;
		}
	for (int i = 0; i < 160; i++)
		for (int j = 0; j < 4; j++)
		{
			orient_block[i][j] = out_mat[k][block];
			k++;
		}
	for (int i = 0; i < 160; i++)
		for (int j = 0; j < 3; j++)
		{
			pos_block[i][j] = out_mat[k][block];
			k++;
		}
}
//
...
{
	static fp state[22];
	static fp state_out[22];
	static fp p[22][22];
	static fp p_out[22][22];
	static fp accel_block[160][3];
	static fp gyro_block[160][3];
	static fp mag_block[160][3];
	static fp gps_pos[3];
	static fp gps_vel[3];

	static fp orient_block[160][4];
	static fp orient_block_out[160][4];
	static fp pos_block[160][3];
	static fp pos_block_out[160][3];

	static fp in_mat[1952][50];
	static fp out_mat[1626][50];
...
#pragma HLS ARRAY_PARTITION variable=out_mat complete
	...
	cout << "-----------------------------------------------" << endl;
	//
	FILE *fp_in = fopen("inp.bin", "rb");
	if (fp_in == NULL)printf("inp.bin!\n");
	for (int i = 0; i < 50; i++)
		for (int j = 0; j < 1952; j++)
			fread(&in_mat[j][i], sizeof(float),1,fp_in);
	fclose(fp_in);
	//
	FILE *fp_out = fopen("outp.bin", "rb");
	if (fp_out == NULL)printf("outp.bin!\n");
	for (int i = 0; i < 50; i++)
		for (int j = 0; j < 1626; j++)
			fread(&out_mat[j][i], sizeof(float),1,fp_in);
	fclose(fp_out);
	//
	for (int block = 0; block < 50; block++)                                                  //50
	{
		cout << "block = "  << block << endl;
		read_inp
		(
				block,in_mat,state,
				p,accel_block,gyro_block,mag_block,gps_pos,gps_vel
		);
		//
		read_outp
		(
				block,out_mat,
				state_out,p_out,orient_block,pos_block
		);
		//
		fp mag_last[3] = {mag_block[159][0],mag_block[159][1],mag_block[159][2]};
		fusion(state,p,accel_block,gyro_block,mag_last,gps_pos,gps_vel,orient_block_out,pos_block_out);
		//
		for (int i = 0; i < 4; i++)
			for (int j = 0; j < 160; j++)
			{
				fp diff1 = fabs(100*(orient_block[j][i] - orient_block_out[j][i])/orient_block[j][i]);
				if(diff1 > 1e-4)
		...
		for (int i = 0; i < 3; i++)
			for (int j = 0; j < 160; j++)
			{
				fp diff2 = fabs(100*(pos_block[j][i] - pos_block_out[j][i])/pos_block[j][i]);
				if(diff2 > 1e-4)
		...
		{
			//cout << "i = "<< i  << endl;
			fp diff3 = fabs(100*(state[i] - state_out[i])/state[i]);
			if(diff3 > 1e-3)
			{

				cout << "diff3 = "<< diff3  << endl;
				cout << " = "<< state[i]  << " = "<< state_out[i]  << endl;
			}
		}
		//
	}
